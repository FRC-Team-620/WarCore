...
public class AutoTunable {
	...
	public static class TuneableSender implements Sendable {
		...
		public void close() { // Note this fails to close everything if live window mode has been entered.
								// Controlable gets modifed and stays
			...
			NetworkTableInstance.getDefault().getEntry("/SmartDashboard/" + this.path + "/.name").unpublish(); // TODO:
																												// Allow
																												// for
																												// configuration
																												// of
		...
		private void addtoBuilder(SendableBuilder builder, TuneableField tf) {
			try {
				...
				if (rawVal instanceof Sendable) {
					// SmartDashboard.putData(path + "/" + key, (Sendable)rawVal); //TODO: This is
					// bad because it is putting data to smart dashbaord before publishing. We
					// should put inside of the initbuilder method
			...
			try {
				for (var methods : builder.getClass().getMethods()) { // correctly builds all array properties. This
					...
					if (methods.getParameterCount() == 3 && methods.getParameterTypes()[0] == String.class
						...
						methods.invoke(builder, tf.f.getName(), supplier, consumer);
		...
		try {
			...
			if (foundFields.size() != 0) {
				...
				var sendable = new TuneableSender(path, foundFields); // This has a bug where if the sender is unable to
																		// display a field it will not be counded
																		// leading
																		// to blank sendables.
				sendable.publish();
